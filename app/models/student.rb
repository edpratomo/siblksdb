class Student < ActiveRecord::Base
  ...
  def self.get_active_stats_for_program prg, last_x_months=6
    now = DateTime.now.in_time_zone
    (0..(last_x_months - 1)).to_a.reverse.inject({}) do |m,o|
      dt = now - o.month
      month, year = dt.month, dt.year
      ...
      active_students = StudentsRecord.joins(:student).
        joins("JOIN pkgs ON pkg_id = pkgs.id").
        joins("JOIN courses on pkgs.course_id = courses.id").
        joins("JOIN programs ON courses.program_id = programs.id").
        where("programs.id = ? AND started_on < ? AND (status = 'active' OR finished_on > ?)",
        prg, dt.end_of_month, dt.end_of_month).
        group(:sex).count
